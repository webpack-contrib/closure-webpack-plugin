 ...
 * **exportGlobalGoog - (optional) boolean.  If true, then the global (window scoped) `goog` variable will be assigned.  If false, then it will not be assigned.  If unspecified, then the global `goog` variable will be assigned only if the webpack build mode is not `production`.
