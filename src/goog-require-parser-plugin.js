...
class GoogRequireParserPlugin {
  constructor(options) {
    ...
    Object.keys(this.options.extraDeps).forEach((namespace) => {
      this.googPathsByNamespace.set(namespace, this.options.extraDeps[namespace]);
    });
      ...
      walk.simple(ast, {
        CallExpression(node) {
          if (node.callee.type === 'MemberExpression' &&
            ...
            if (!googDepsByPath.has(filePath) && node.arguments[2] && node.arguments[2].elements.length > 0) {
              googDepsByPath.set(filePath, node.arguments[2].elements.map(node => node.value));
            }
          }
        },
      });
  ...
  apply(parser) { // eslint-disable-line class-methods-use-this
    parser.plugin(['call goog.require', 'call goog.module.get', 'call goog.provide'], (expr) => {
      if (!parser.state.current.hasDependencies(dep => dep.request === this.basePath)) {
        const baseInsertPos = this.options.mode === 'NONE' ? 0 : null;
        parser.state.current.addDependency(new GoogDependency(this.basePath, baseInsertPos));
      }
      ...
      try {
        ...
        const insertPosition = this.options.mode === 'NONE' ? expr.start : null;
        if (this.googDepsByPath.has(modulePath)) {
          this.googDepsByPath.get(modulePath).forEach((dep) => {
            ...
            parser.state.current.addDependency(new GoogDependency(depPath, insertPosition));
          });
        }
        parser.state.current.addDependency(new GoogDependency(modulePath, insertPosition));
      } catch (e) {
        console.error(e);
      }
      return false;
    ...
    if (this.options.mode === 'NONE') {
      parser.plugin('statement', (expr) => {
        if (expr.type === 'VariableDeclaration' && expr.declarations.length === 1 &&
          expr.declarations[0].id.name === 'goog' && parser.state.current.userRequest === this.basePath) {
          parser.state.current.addVariable('goog', 'window.goog', []);
          parser.state.current.contextArgument = function () { // eslint-disable-line no-param-reassign
            return 'window';
          };
          const { variableInjectionFunctionWrapperEndCode } = parser.state.current;
          // eslint-disable-next-line no-param-reassign
          parser.state.current.variableInjectionFunctionWrapperEndCode = function (varExpressions, block) {
            return `window.goog = goog;${variableInjectionFunctionWrapperEndCode.call(this, varExpressions, block)}`;
          };
        }
      });
    }
