...
function getDepsBlockPromise(depBlock, outputOptions, requestShortener, name) {
  if (depBlock.chunks) {
    ...
    const shortChunkName = pathChunkCheck ? `/*! ${requestShortener.shorten(depBlock.chunkName)} */` : '';
    const chunkIdsList = chunks.map(chunk => JSON.stringify(chunk.id)).join(', ');
...
class ClosureCompilerImportDependencyTemplate extends ImportDependency.Template {
  apply(dep, source, outputOptions, requestShortener) {
    ...
    const promise = getDepsBlockPromise(depBlock, outputOptions, requestShortener, 'import()');
  ...
  getContent(promise, dep, comment) { // eslint-disable-line class-methods-use-this
